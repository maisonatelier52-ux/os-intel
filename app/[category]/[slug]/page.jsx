...
export async function generateMetadata({ params }) {
  ...
  // ✅ metaTitle is the SEO title — keep it short and keyword-rich
  // Format: "{Short keyword title} | {Category} — OS-Intel"
  const metaTitle =
    article.metaTitle ||
    `${article.title.slice(0, 60)} | ${formattedCategory} — ${SITE_NAME}`;

  // ✅ metaDescription from JSON (includes excerpt with all keywords)
  const metaDescription =
    article.metaDescription ||
    article.excerpt?.slice(0, 155) ||
    `Read ${article.title} on ${SITE_NAME}.`;
  ...
  switch (item.type) {
    case "paragraph":
      return (
        <p
          key={index}
          className="text-gray-700 text-sm font-serif leading-relaxed mb-4"
        >
          {item.text}
        </p>
      );
    ...
    case "image":
      return (
        <div
          key={index}
          className="relative w-full h-[270px] md:h-[420px] overflow-hidden mb-6"
        >
          <Image
            src={item.src}
            alt={item.alt}
...
export default async function ArticleDetailPage({ params }) {
  ...
  // ✅ SEO: Use metaTitle as H1 (short, keyword-rich, ~52 chars)
  // Use article.title as H2 subtitle (the full descriptive title)
  const seoH1 =
    article.metaTitle ||
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: article.title,
        item: `${SITE_URL}/${categoryKey}/${slug}`,
      },
    ],
  };
          ...
          <div className="flex flex-col md:flex-row md:items-start gap-10">
            ...
            <article className="w-full md:w-[75%]">
              ...
              {/*
                ...
                Problem: article.title is 85 chars (too long) and missing
                keywords: geneva, amid, warships, intel

                Solution:
                  H1 = metaTitle (short ~52 chars, has all keywords)
                  H2 = article.title (full descriptive title, still indexed)

                Title tag: "US-Iran Nuclear Talks in Geneva Amid Warships | OS-Intel"
                H1 (metaTitle): "US-Iran Nuclear Talks in Geneva Amid Warships | OS-Intel"
                H2 (article.title): full long title for readers
              ...
              */}
              ...
              {/*
                ── PREV / NEXT ───────────────────────────────────────────────
                ✅ HEADING ORDER FIX: h4 → h3
                Prev/Next sit under the article body (no h2 wrapper needed).
                Using h3 keeps the order: h1 → h2 (subtitle) → h3 (prev/next titles)
              */}
              <div className="flex flex-col sm:flex-row gap-7 md:gap-4 mb-10">
                {prevPost ? (
                  <Link
                  ...
                  >
                    ...
                    {/* ✅ h3 instead of h4 */}
                    <h3 className="text-gray-900 text-sm font-bold font-serif leading-snug group-hover:text-red-600 transition-colors ml-5">
                      {prevPost.title}
                    </h3>
                  </Link>
                ) : (
                ...
                {nextPost ? (
                  ...
                  >
                    ...
                    {/* ✅ h3 instead of h4 */}
                    <h3 className="text-gray-900 text-sm font-bold font-serif leading-snug group-hover:text-red-600 transition-colors mr-5">
                      {nextPost.title}
                    </h3>
                  </Link>
                ) : (
              ...
              {morePosts.length > 0 && (
                <div className="mb-10">
                  {/* ✅ h2 section heading — correct level */}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    {morePosts.map((post) => (
                      ...
                      >
                        ...
                        {/* ✅ h3 instead of h4 — under h2 "More posts" */}
                        <h3 className="text-gray-900 text-sm font-black font-serif leading-snug group-hover:text-red-600 transition-colors">
                          {post.title}
                        </h3>
                      </Link>
                    ))}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-3 mt-2 justify-center md:justify-start">
                      {author.social?.twitter && (
                        <a href={author.social.twitter} target="_blank" rel="noopener noreferrer" title={`Follow ${author.name} on X`} aria-label="X" className="hover:text-red-500 transition-colors">
                          <FaXTwitter size={15} />
                        </a>
                      )}
                      {author.social?.reddit && (
                        <a href={author.social.reddit} target="_blank" rel="noopener noreferrer" title={`Follow ${author.name} on Reddit`} aria-label="Reddit" className="hover:text-red-500 transition-colors">
                          <FaReddit size={15} />
                        </a>
                      )}
                      {author.social?.quora && (
                        <a href={author.social.quora} target="_blank" rel="noopener noreferrer" title={`Follow ${author.name} on Quora`} aria-label="Quora" className="hover:text-red-500 transition-colors">
                          <FaQuora size={15} />
                        </a>
                      )}
                      {author.social?.medium && (
                        <a href={author.social.medium} target="_blank" rel="noopener noreferrer" title={`Follow ${author.name} on Medium`} aria-label="Medium" className="hover:text-red-500 transition-colors">
                          <SiMedium size={15} />
                        </a>
                      )}
                    </div>
